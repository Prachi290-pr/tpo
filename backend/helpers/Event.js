  ...
  try {
    ...
    const [result] = await connection.query(
      ...
      SELECT 
        ...
        e.eventDeadline
      FROM tpo_events e
      JOIN student_details s ON FIND_IN_SET(trim(s.branch), e.department) > 0
        AND FIND_IN_SET(trim(s.degree_year), e.eligible_degree_year) > 0
      WHERE s.id = ?
