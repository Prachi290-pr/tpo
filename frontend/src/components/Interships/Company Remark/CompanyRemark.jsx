...
import api from "../../../api";
import InternAddRemarkForm from "./AddRemarkForm";

const InternCompanyRemark = () => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const response = await api.get("companies/intership/getAll");
  ...
  const handleCompanyChange = async (e) => {
    ...
    try {
      const response = await api.get(`companies/intership/${companyName}/years`);
  ...
  const handleDisplayRemarks = async () => {
    ...
    try {
      const response = await api.get(`companies/intership/${selectedCompany}/remarks`);
  ...
  const handleAddRemark = async (newRemark) => {
    ...
    try {
      ...
      const response = await api.post(
        `companies/intership/${selectedCompany}/remarks`,
        JSON.stringify(newRemark),
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const handleUpdateStatus = async (remarkId, newStatus) => {
    try {
      await api.put(`companies/intership/${selectedCompany}/remarks/${remarkId}`, {
        status: newStatus,
      });
...
export default InternCompanyRemark;
