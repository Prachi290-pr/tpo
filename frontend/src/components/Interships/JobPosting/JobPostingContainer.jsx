...
// import api from "../../../api";
import IntershipCustomQuestionsForm from "./CustomQuestionsForm";
import IntershipQuestionTable from "./QuestionTable";
import api from "../../../api";
import IntershipRegistrationForm from "./RegistrationForm";
...
const IntershipPost = () => {
  ...
  useEffect(() => {
    const fetchConfirmedCompanies = async () => {
      try {
        const response = await api.get("/remarks/intership/confirmed");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await api.post("jobpostings/intership", formDataToSend, {
        headers: {
    ...
    <div className="flex items-center justify-center bg-white-100 py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl w-full space-y-4 bg-white p-10 shadow-lg rounded-lg">
        ...
        {selectedCompany.companyId && (
          <>
            <IntershipRegistrationForm onFormDataChange={handleFormDataChange} />
          
            {withTracker && (
              <>
                <IntershipCustomQuestionsForm onAddQuestion={handleAddQuestion} />
                {questions.length > 0 && <IntershipQuestionTable questions={questions} onDeleteQuestion={handleDeleteQuestion}  />}
              </>
            )}
            <div className="space-y-4">
              <button
