...
const ERPApti = () => {
  ...
  const fetchERPCompanies = () => {
    api
    .get(`/erp/geterpcompanyreportApi?id=${localStorage.getItem('AYearfor')}`)
    .then((res) => {
  ...
  const fetchERPDetails = (id) => {
    api
      .get(`/erp/geterpdetailsApti?id=${id}`)
      .then((res) => {
  ...
  <table className="w-full">
    <thead className="bg-gray-800 sticky top-0 z-8">
      ...
      <tr className="sticky bg-gray-800 z-8">
        {filteredCompanies.map((company, index) => (
