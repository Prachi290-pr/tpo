...
const InterViewReport = () => {
  ...
  const fetchCompanies = () => {
    api
      .get("/studentgen/getInterViewTitle")
      .then((res) => {
  ...
  const fetchRegistered = (titles) => {
    api
      .post(`/studentgen/getInterViewReportByTitle`,{ques_title:titles})
      .then((res) => {
  ...
  const downloadExcel = () => {
    const worksheetData = [];
      ...
      // Add the header row
      worksheetData.push(['College Id', 'Name', 'Branch','Total Marks', 'Obtained Marks']);
  ...
  return (
    <div className='container mx-auto mt-5'>
      <Select 
        ...
        options={companyDetails.map(company => ({ value: company.title, label: `${company.title}` }))}
 ...
 <tr>
   ...
   {!hasTrackerQuestion && (
     <>
       ...
       <th className="px-4 py-2 text-center text-sm font-medium text-white">Total Marks</th>
       <th className="px-4 py-2 text-center text-sm font-medium text-white">Obtained Marks</th>
     </>
   )}
 </tr>
</thead>
