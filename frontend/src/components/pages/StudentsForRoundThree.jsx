...
const StudentsForRoundThree = () => {
  ...
  useEffect(() => {
    fetchRoundThreeStudents(companyId);
    fetchRoundTwoStudentIds(companyId);
    fetchPlacedStudentIds(companyId);
    fetchRoundFourStudentIds(companyId);
  }, [companyId]);
  ...
  const fetchRoundThreeStudents = (companyId) => {
    api
      ...
      .catch((err) => {
        console.error("Error fetching round 3 students:", err);
      });
  };

  const fetchRoundTwoStudentIds = (companyId) => {
    api
      .get(`drivestatus/getround2ids/${companyId}`)
      .then((res) => {
        const roundTwoIds = res.data.map((item) => item.driveid);
        setRoundTwoStudentIds(roundTwoIds);
      })
      .catch((err) => {
        console.error("Error fetching round 2 student IDs:", err);
      });
  };
  ...
  const handleDeleteFromPlaced = (studentId) => {
    api
      ...
      .catch((err) => {
        console.error(`Error moving student ${studentId} back to round two:`, err);
      });
  };
  ...
  const handleGoBack = () => {
    navigate(`/driveStatus/students?companyId=${companyId}`);
  ...
  const downloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      roundThreeStudents.map((student, index) => ({
        "Sr. No.": index + 1,
    ...
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-4">
            ...
            {isDropdownOpen && (
              <div className="absolute mt-1 w-44 bg-white divide-y divide-gray-100 rounded-lg shadow-md z-10">
                <ul className="py-2 text-sm text-gray-700">
                  <li>
                    <Link
                      to={`/drivestatus/students?companyId=${companyId}`}
          ...
          <Link
            to={`/drivestatus/students-for-round-two?companyId=${companyId}`}
            className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 inline-block"
          >
            View Round Two Students
          </Link>
        ...
        <table className="">
          <thead className="sticky top-0 bg-gray-800 z-8 w-fulls">
            <tr className="bg-gray-100 text-gray-600 uppercase text-sm leading-normal">
              ...
              <th className="py-3 px-6 text-left border-b text-black bg-gray-200">Select For Round Two</th>
