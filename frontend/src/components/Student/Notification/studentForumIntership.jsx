...
const StudentForumPageInter = ({ uid }) => {
  ...
  useEffect(() => {
    if (selectedCompany) {
      api
        .get(`/forums/chats/intership/${selectedCompany.id}`)
        .then((res) => {
          setChats(res.data);
        })
        .catch((err) => {
...
export default StudentForumPageInter;