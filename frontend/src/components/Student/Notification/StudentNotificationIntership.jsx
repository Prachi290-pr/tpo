...
import api from "../../../api";
import StudentForumPageInter from "./studentForumIntership";


const StudentNotificationIntership = ({uid}) => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const response = await api.get("jobpostings/company-name-inter");
  ...
  const fetchAnnouncements = async (jobPostingId) => {
    try {
      const response = await api.get(`announcements/intership/${jobPostingId}`);
...
try {
  const response = await api.get(`forums/chats/intership/${jobPostingId}`);
  if (response.status === 200) {
    if (response.data && response.data.length > 0) {
...
const  handleQuestionSubmit = async () => {
  ...
  try {
    const response = await axios.post('http://localhost:3000/forums/chats', data);
...
export default StudentNotificationIntership;
