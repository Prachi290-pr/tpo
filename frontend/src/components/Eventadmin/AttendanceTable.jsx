...
function AttendanceTable() {
  ...
  const uniqueEvents = [...new Set(attendanceData.flatMap(item => Object.keys(item.events)))];
    ...
    newData = newData.map(item => {
      const filteredEvents = Object.entries(item.events).reduce((acc, [eventName, eventDetails]) => {
        if (eventName === eventFilter) {
          acc[eventName] = eventDetails;
        }
        return acc;
      }, {});
      return { ...item, events: filteredEvents };
    }).filter(item => Object.keys(item.events).length > 0);
  }
  ...
  useEffect(() => {
    handleFilterChange();
  }, [degreeYearFilter, branchFilter, batchFilter,eventFilter]);
  ...
  return (
    <div className="flex flex-col gap-8 justify-start items-start  w-auto p-10 bg-white rounded-2xl shadow-2xl ">
      <h1 className="text-5xl font-extrabold text-gray-800 mb-8 tracking-wider border-b pb-4 border-gray-200 w-full">
        Attendance Management
      </h1>
  
      <div className="mb-6 flex flex-col lg:flex-row md:flex-row gap-6 items-center w-full ">
        ...
        <div className="flex flex-col w-1/3">
  <label className="text-gray-600 font-semibold mb-2">Event</label>
  <select 
    value={eventFilter} 
  ...
  >
    ...
    {uniqueEvents.map((event, index) => (
      ...
      {filteredData.length > 0 ? (
        <div className="overflow-x-auto overflow-y-scroll max-h-[600px] relative w-full rounded-lg shadow-2xl border border-gray-200 bg-gradient-to-r from-gray-100 to-gray-50">
          <table className="table-auto border-collapse w-full text-left bg-white relative">
            <thead className="bg-blue-700 sticky text-white top-0 z-8">
              <tr>
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">Student ID</th>
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">College ID</th>
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">Name</th>
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">Branch</th>
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">Degree Year</th>
                {Object.keys(filteredData[0].events).map((event, index) => (
                  <React.Fragment key={index}>
                    <th colSpan={3} className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">
                      <div className="border-b pb-2 text-white">{event}</div>
                      {console.log(event)}
                    </th>
                  </React.Fragment>
                ))}
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">Total E</th>
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">Total R</th>
                <th className="border px-6 py-4 text-sm font-semibold tracking-wide text-white">Total P</th>
              </tr>
              <tr>
                ...
                {Object.keys(filteredData[0].events).map((event, index) => (
                  <React.Fragment key={index}>
                    <th className="border px-6 py-2 text-sm font-semibold tracking-wide text-white">E</th>
                    <th className="border px-6 py-2 text-sm font-semibold tracking-wide text-white">R</th>
                    <th className="border px-6 py-2 text-sm font-semibold tracking-wide text-white">P</th>
                  </React.Fragment>
                ))}
                <th className="border"></th>
                <th className="border"></th>
