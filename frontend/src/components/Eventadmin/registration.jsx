...
function RegistrationPage() {
  ...
  const approveStudent = async (student_id, emailId, isOnline, eventLink) => {
    setActionStatus({ ...actionStatus, [student_id]: 'approving' });
    console.log(student_id)
    try {
      if (isOnline === 1 && eventLink != null){
        await nodeApi.put(`/approveStudent/${eventId}`, { student_id: student_id });
        await nodeApi.post(`/sendAttendanceQrcode`, { email: emailId, src: eventLink, isOnline: true});
  
        setRegistrations(registrations.map(reg =>
          reg.student_id === student_id ? { ...reg, isApproved: true } : reg
        ));
        alert("Student approved successfully!");

      } else {
        
        const qrCodeData = JSON.stringify({ event_id: eventId, student_id: student_id });
      ...
      setActionStatus({ ...actionStatus, [student_id]: null });
    }
  };
  ...
  const resetEventAttendance = async () =>{
    try{
      await nodeApi.put(`/resetEventAttendance`, { event_id : eventId });
      alert("Attendance reset successful !");

    }catch(err){
      setError("Error resetting attendance" , err.message);
    }
  }
  ...
  const downloadSheet = () => {
    ...
    const sheetData = registrations.map(({ clg_id,branch, email_id, first_name, last_name, transaction_id, isApproved },index) => ({
      "Sr. No.": index+1,
      "College ID": clg_id,
    ...
    XLSX.writeFile(workbook, `Registrations_${eventId}.xlsx`);
  };
      ...
      <div className="flex flex-col gap-4 ml-[18rem] md:mt-32 w-[70%]  px-4 registrationshow">
        ...
        {registrations.length ? (
          <div className="flex gap-4 mb-4">
            ...
            <button
              onClick={resetEventAttendance}
              disabled={bulkActionStatus === 'resetting'}
              className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 transition"
            >
              {bulkActionStatus === 'resetting' ? <CircularProgress size={20} color="inherit" /> : 'Attendance Reset'}
            </button>
          ...
          <div className="w-full relative rounded-xl tablediv">
            <table className="w-[100%]  tablereg divide-y divide-gray-200 bg-white shadow-md rounded-xl">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {registrations.map(({ student_id, branch,clg_id, email_id, first_name, last_name, transaction_id, isApproved, isOnline, eventLink },index) => (
                  <tr key={student_id} className="hover:bg-gray-50">
                    <td className="px-4 py-2 text-sm text-gray-700">{index+1}</td>
                    <td className="px-4 py-2 text-sm text-gray-700">{clg_id}</td>
                    <td className="px-4 py-2 text-sm text-gray-700">{branch}</td>
